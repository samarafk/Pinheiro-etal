...
There is a total of `r dim(DataInfoNew)[1]` sequences from `r length(unique(DataInfoNew$Patient))` patients (`r length(unique(DataInfoNew$Patient[DataInfoNew$Group=="SP"]))` SP, `r length(unique(DataInfoNew$Patient[DataInfoNew$Group=="RP"]))` RP and `r length(unique(DataInfoNew$Patient[DataInfoNew$Group=="P"]))` P).

Table 4 of the manuscript: Distribution of sequences per individual:
```{r,echo=FALSE}
xx <- aggregate(DataInfoNew$Accession,by=list(Group=DataInfoNew$Group,Date=DataInfoNew$Date,Patient=DataInfoNew$Patient),length)
xx <- xx[order(xx[,1]),]
colnames(xx) <- c("Group","Date","Patient","Freq")
library(knitr)
kable(xx, format = "markdown",row.names=FALSE)
